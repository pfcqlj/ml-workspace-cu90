...
RUN apt-get update && apt-get install -y --no-install-recommends ca-certificates apt-transport-https curl && \
...
RUN \
    ...
    conda install -y cudatoolkit=9.0 -c pytorch && \
    # Install cupy: https://cupy.chainer.org/
    pip install --no-cache-dir cupy-cuda90 && \
    ...
    pip install --no-cache-dir mxnet-cu90mkl==1.0.0 && \
